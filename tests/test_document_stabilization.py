...
def test_list_documents(client):
    ...
    with patch("src.api.v1.documents.list_documents", new_callable=AsyncMock) as mock_list:
...
def test_patch_document_metadata_success(client):
    ...
    with patch("src.api.v1.documents.get_document_by_id", new_callable=AsyncMock) as mock_get, \
         patch("src.api.v1.documents.update_document_metadata", new_callable=AsyncMock) as mock_update:
...
def test_reingest_document_success(client):
    ...
    with patch("src.api.v1.documents.get_document_by_id", new_callable=AsyncMock) as mock_get, \
         patch("src.api.v1.documents.create_ingestion_job", new_callable=AsyncMock) as mock_create_job, \
         patch("src.api.v1.documents.process_job", new_callable=AsyncMock) as mock_process:
...
def test_replace_file_success(client):
    ...
    with patch("src.api.v1.documents.get_document_by_id", new_callable=AsyncMock) as mock_get, \
         patch("src.api.v1.documents.upload_object") as mock_upload, \
         patch("src.api.v1.documents.update_document_source", new_callable=AsyncMock) as mock_update_src, \
         patch("src.api.v1.documents.create_ingestion_job", new_callable=AsyncMock) as mock_create_job, \
         patch("src.api.v1.documents.process_job", new_callable=AsyncMock) as mock_process:
        ...
        file_payload = {"file": ("new_version.pdf", b"%PDF-1.4 mock new content", "application/pdf")}
...
def test_archive_and_restore_document(client):
    ...
    with patch("src.api.v1.documents.get_document_by_id", new_callable=AsyncMock) as mock_get, \
         patch("src.api.v1.documents.archive_document", new_callable=AsyncMock) as mock_archive, \
         patch("src.api.v1.documents.restore_document", new_callable=AsyncMock) as mock_restore:
        
        mock_get.side_effect = [mock_doc, {**mock_doc, "status": "archived"}, {**mock_doc, "status": "processed"}]
...
def test_get_document_jobs(client):
    ...
    with patch("src.api.v1.documents.get_document_by_id", new_callable=AsyncMock) as mock_get, \
         patch("src.api.v1.documents.get_jobs_by_document_id", new_callable=AsyncMock) as mock_jobs_list:
...
def test_list_ingestion_jobs(client):
    ...
    with patch("src.api.v1.ingestion.list_ingestion_jobs", new_callable=AsyncMock) as mock_list:
...
def test_get_job_details(client):
    ...
    with patch("src.api.v1.ingestion.get_job_details_by_id", new_callable=AsyncMock) as mock_get:
...
def test_retry_job_success(client):
    ...
    with patch("src.api.v1.ingestion.retry_ingestion_job", new_callable=AsyncMock) as mock_retry, \
         patch("src.api.v1.ingestion.process_job", new_callable=AsyncMock) as mock_process, \
         patch("src.api.v1.ingestion.get_job_details_by_id", new_callable=AsyncMock) as mock_get:
        
        mock_retry.return_value = True
        mock_process.return_value = {"status": "completed"}
        mock_get.return_value = mock_job

        response = client.post("/api/v1/ingestion/jobs/job-1/retry?process_immediately=true")
        assert response.status_code == 200
        data = response.json()
        assert data["job_id"] == "job-1"
        assert data["status"] == "completed"
