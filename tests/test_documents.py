...
def test_ingest_document_success(client):
    ...
    with patch(
        "src.api.v1.documents.check_object_exists", return_value=True
    ) as mock_minio, patch(
        "src.api.v1.documents.create_ingestion_job", new_callable=AsyncMock
    ) as mock_db:

        mock_db.return_value = ("doc-uuid-1234", "job-uuid-5678", "pending")
...
def test_ingest_document_missing_minio_object(client):
    ...
    with patch(
        "src.api.v1.documents.check_object_exists", return_value=False
    ) as mock_minio:
        response = client.post("/api/v1/documents/ingest", json=payload)
...
def test_search_documents_success(client):
    ...
    with patch("src.services.search_service.generate_embeddings") as mock_embed, patch(
        "src.services.search_service.search_document_chunks", new_callable=AsyncMock
    ) as mock_db:
        ...
        data = response.json()
        assert (
            data["query"] == "What is the role of OpenClaw in the leadgen architecture?"
        )
        ...
        mock_embed.assert_called_once_with(
            ["What is the role of OpenClaw in the leadgen architecture?"]
        )
        mock_db.assert_called_once()
...
async def test_search_document_chunks_sql_construction():
    ...
    with patch(
        "src.services.database.asyncpg.connect",
        new_callable=AsyncMock,
        return_value=mock_conn,
    ) as mock_connect:
        filters = DocumentSearchFilters(type="case", tags=["Machine Learning"])

        await search_document_chunks(
            query_embedding=[0.1] * 1536, limit=5, filters=filters
        )
