...
def extract_text_from_xlsx(file_bytes: bytes) -> str:
    ...
    for sheet in wb.worksheets:
        ...
        for r_idx, row in enumerate(sheet.iter_rows(values_only=True), start=1):
            ...
            # Skip fully empty rows
            if not row or not any(val is not None and str(val).strip() != "" for val in row):
                continue
...
def extract_text(file_bytes: bytes, filename: str, mime_type: Optional[str] = None) -> str:
    ...
    logger.info(f"Extracting text from file '{filename}' with extension '{ext}' and MIME type '{mime_type}'")
    
    # Check physical file size limit in MB
    file_size_mb = len(file_bytes) / (1024 * 1024)
    if file_size_mb > settings.MAX_FILE_SIZE_MB:
        raise ValueError(f"File size exceeds safety limit of {settings.MAX_FILE_SIZE_MB} MB.")
