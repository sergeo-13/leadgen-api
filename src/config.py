...
class Settings(BaseSettings):
    ...
    # Hermes
    HERMES_API_BASE_URL: str = os.getenv(
        "HERMES_API_BASE_URL", "http://hermes-gateway:8642"
    )
    ...
    ENTRA_AUTHORITY: str = os.getenv(
        "ENTRA_AUTHORITY", "https://login.microsoftonline.com/organizations"
    )
    ENTRA_REDIRECT_URI: str = os.getenv("ENTRA_REDIRECT_URI", "")
    ENTRA_POST_LOGOUT_REDIRECT_URI: str = os.getenv(
        "ENTRA_POST_LOGOUT_REDIRECT_URI", ""
    )

    # Session
    AUTH_SESSION_SIGNING_SECRET: str = os.getenv("AUTH_SESSION_SIGNING_SECRET", "")
    AUTH_SESSION_MAX_AGE_SECONDS: int = int(
        os.getenv("AUTH_SESSION_MAX_AGE_SECONDS", "28800")
    )
    AUTH_SESSION_COOKIE_SECURE: bool = (
        os.getenv("AUTH_SESSION_COOKIE_SECURE", "true").lower() == "true"
    )
    AUTH_SESSION_COOKIE_NAME: str = os.getenv(
        "AUTH_SESSION_COOKIE_NAME", "leadgen_session"
    )
    AUTH_LOGIN_TRANSACTION_TTL_SECONDS: int = int(
        os.getenv("AUTH_LOGIN_TRANSACTION_TTL_SECONDS", "600")
    )
    AUTH_TOKEN_CLOCK_SKEW_SECONDS: int = int(
        os.getenv("AUTH_TOKEN_CLOCK_SKEW_SECONDS", "120")
    )
